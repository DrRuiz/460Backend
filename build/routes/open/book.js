...
const allButId = 'SELECT isbn13, authors, publication_year, original_title, title, rating_avg, rating_count,' +
 ...
 * },]"
 */
bookRouter.get('/getAll', (request, response) => {
 ...
 */
bookRouter.get('/authortitle', (request, response, next) => {
    ...
    utilities_1.pool.query(query, values)
        .then((result) => {
 ...
 */
bookRouter.get('/ratings', (request, response, next) => {
    if (isNumberProvided(request.query.min) &&
 ...
 * @apiError (404: Book does not exist) {String} message "Book title does not exist in database."
 */
bookRouter.get('/title', (request, response, next) => {
 ...
 */
bookRouter.get('/year', (request, response, next) => {
